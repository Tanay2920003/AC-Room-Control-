...
int buttonState;
int lastButtonState = HIGH; // Assuming the button is normally open
unsigned long lastDebounceTime = 0;  // Last time the button was pressed
unsigned long debounceDelay = 50;    // Debounce time in milliseconds

// Define system pause state
...
void loop() {
  ...
  if (!systemPaused && roomTemperature <= setTemperature) {
  ...
  // If the button state has been stable for the debounce delay, we consider it to be "pressed"
  if ((millis() - lastDebounceTime) > debounceDelay) {
    ...
    if (reading != buttonState) {
      ...
      if (buttonState == HIGH) {
        ...
        // If system is paused, turn off the relay
        if (systemPaused) {
          digitalWrite(RELAY_PIN, LOW);
        }
      }
  ...
  // Add some delay to prevent rapid sensor readings
  delay(100);
}
